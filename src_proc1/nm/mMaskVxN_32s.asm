...
global _mMaskVxN_32s:label;
// void mMaskVxN_32s(nm16s* pTriangles, nm16s* pMask, nm16s** pROI, int ImageStride, int* pTrianglesHeight, int* pTrianglesWidth, int count);
...
<_mMaskVxN_32s>
	...
	ar2 = [--ar5];		// **pROI
	gr5 = [--ar5];		// ImageStride
	ar3 = [--ar5];		// *pTriangsHeight
	ar4 = [--ar5];		// *pTriangsWidth
...
<NextTrMaskROI>
	gr6 = [ar3++];					// читаем высоту(маски, изображжения с треугольником и ROI) из массива выстот pTriagsHeight
	gr7 = [ar4++];					// читаем ширину(маски, изображжения с треугольником и ROI) из массива ширин pTriagsWidth
	...
	delayed goto ar6 + gr6 with gr7 >>= 1;						// pTriagsWidth[i] >> 1
		with gr0 = gr7 << 1;
		with gr1 = gr0;
