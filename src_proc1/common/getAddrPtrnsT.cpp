...
extern clock_t t0Wait, t1Wait;
extern int waitingTime;
#define CHECK_STATUS(a) t0Wait = clock(); while (!msdGetStatusCopy(points[a], 0)); t1Wait = clock(); waitingTime += t1Wait-t0Wait
//#define CHECK_STATUS(a) msdWaitIndexCopy(points[a], 0)
...
SECTION(".text_demo3d") int getAddrPtrnsT(NMGL_Context_NM1* context, Patterns* patterns, Polygons* poly){
	...
	//ADD_COPY(poly->windowPtrns, polyTmp->windowPtrns, 4 * size, 13);
	...
	/*CHECK_STATUS(13);
	for (int i = 0; i < size; i++) {
		int p1 = polyTmp->pointInImage[i] & 1;
		polyTmp->pointInImage[i] -= p1;
		polyTmp->windowPtrns[i].width += p1;
		polyTmp->windowPtrns[i].width += polyTmp->windowPtrns[i].width & 1;
		polyTmp->windowPtrns[i].x0 -= p1;
	}*/
