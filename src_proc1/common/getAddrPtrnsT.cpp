...
SECTION(".text_demo3d") int getAddrPtrnsT(NMGL_Context_NM1* context, Patterns* patterns, Polygons* poly){
	...
	nmppsCopy_32s((nm32s*)poly, (nm32s*)polyTmp, 13 * NMGL_SIZE);
	//nmppsCopy_32s(poly->color, context->valuesC, size);
	nmppsCopy_32s(poly->z, context->valuesZ, size);
	nmppsCopy_32s(poly->offsetsY, context->offsetTrY, size);
	nmppsCopy_32s(poly->heights, context->heights, size);
	...
	CHECK_STATUS(4);
	nmppsMulC_AddC_32s(polyTmp->numbersPattrns01, WIDTH_PTRN * HEIGHT_PTRN / 16, (int)ptrnsRight_2s, temp1, size);
	CHECK_STATUS(5);
	nmppsMulC_AddC_32s(polyTmp->numbersPattrns12, WIDTH_PTRN * HEIGHT_PTRN / 16, (int)ptrnsRight_2s, temp2, size);
	CHECK_STATUS(6);
	nmppsMulC_AddC_32s(polyTmp->numbersPattrns02, WIDTH_PTRN * HEIGHT_PTRN / 16, (int)ptrnsRight_2s, temp0, size);
