...
void nmglPushMatrix() {
	...
	if (context->currentMatrixStack->current < context->currentMatrixStack->size - 1) {
		nmblas_scopy(16, (float *)&context->currentMatrixStack->base[context->currentMatrixStack->current], 1,
					 (float *)&context->currentMatrixStack->base[context->currentMatrixStack->current + 1], 1);
