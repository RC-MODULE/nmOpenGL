...
int cullFaceSortTriangles(Triangles* triangles, int count){
	...
	float* walkDirection = cntxt.buffer2 + 3 * NMGL_SIZE;
	nmppsMul_Mul_Sub_32f(triangles->x0, triangles->y1, triangles->x1, triangles->y0, walkDirection, count);
	nmppsMul_Mul_Sub_32f(triangles->x1, triangles->y2, triangles->x2, triangles->y1, cntxt.buffer3, count);
	nmppsAdd_32f(walkDirection, cntxt.buffer3, cntxt.buffer0, count);
	nmppsMul_Mul_Sub_32f(triangles->x2, triangles->y0, triangles->x0, triangles->y2, cntxt.buffer3, count);
	nmppsAdd_32f(cntxt.buffer0, cntxt.buffer3, walkDirection, count);
