...
extern "C" void mMulCVxN_2s16s(nm2s** ppSrcTreangle_2s, int* offsets, int* widths, int* heights, nm16s* pDstTreangle_16s, int* valueC,  int count);
	...
	for(int h=1;h<=32;h++){
		for(int w=0;w<=36;w+=4){
			for(int offset=0; offset<4;offset++){
				...
				t0 = clock();
				mMulCVxN_2s16s((nm2s**)ppSrc,offsets,widths,heights,(nm16s*)dst, valuesC, 32);
				t1 = clock();
