...
SECTION(".data_imu3") float src3[SIZE+2];
SECTION(".data_imu5") float min[SIZE+2];
SECTION(".data_imu6") float max[SIZE+2];
	
	
extern "C" void findMinMax(float* src1, float* src2, float* src3, float* dstMin, float* dstMax, int size);
...
int main()
{
	...
	//nmppsRand_32f(src3, SIZE, 1,1000);
	...
	for(int size=2;size<= SIZE;size+=2){
		t0=clock();
		findMinMax(src1, src2, src3, min, max, size);
		t1=clock();
