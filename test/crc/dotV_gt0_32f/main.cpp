...
#pragma data_section ".data_imu1"		
	v4nm32f src2[NMGL_SIZE];
#pragma data_section ".data_imu2"
	nm32f dst[2 * NMGL_SIZE];
#pragma data_section ".data_imu3"

extern "C" void dotV_gt0_32f(v4nm32f* src1, v4nm32f* src2, nm32f* dstValues, int size);
...
int main()
{
	...
	for(int i=0;i<NMGL_SIZE;i++){
		src1[i].vec[0] = 4*i+0 - 1000;
		src1[i].vec[1] = 4*i+1 - 1000;
		src1[i].vec[2] = 4*i+2 - 1000;
		src1[i].vec[3] = 4*i+3 - 1000;
	...
	for(int localSize=0;localSize<=NMGL_SIZE;localSize++){
		t0 = clock();
		dotV_gt0_32f(src1, src2, dst, localSize);
		t1 = clock();
