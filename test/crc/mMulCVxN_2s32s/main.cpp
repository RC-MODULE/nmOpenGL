...
extern "C" void mMulCVxN_2s32s(nm2s** ppSrcTreangle_2s, int* offsets, int* widths, int* heights, nm32s* pDstTreangle_16s, int* valueC,  int count);
	...
	for(int h=1;h<=32;h++){
		for(int w=0;w<=34;w+=2){
			for(int offset=0; offset<2;offset++){
				...
				t0 = clock();
				mMulCVxN_2s32s((nm2s**)ppSrc,offsets,widths,heights,(nm32s*)dst, valuesC, 32);
				t1 = clock();
