...
SECTION(".text_demo3d") void NMGL_DrawTriangles(NMGL_Context_NM1 *context, NM_Command *command) {
	...
	baseAddrOffs_32s((nm32s*)context->smallFramebuffer.imageBuffer.data, context->imageOffsets, context->imagePoints, poly->count);
	baseAddrOffs_32s((nm32s*)context->smallFramebuffer.depthBuffer.data, context->imageOffsets, context->zBuffPoints, poly->count);
