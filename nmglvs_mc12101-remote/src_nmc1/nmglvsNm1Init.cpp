...
SECTION(".text_nmglvs") int nmglvsNm1Init()
{
	...
	try {
		...
		for (int i = 0; i < 2; i++) {
			int* image = (int*)(imagesData + i);
			for (int j = 0; j < WIDTH_IMAGE * HEIGHT_IMAGE; j++) {
				image[j] = 0;
			}
		}
		defaultFramebuffer.imageBuffer.data = imagesData + 0;
		defaultFramebuffer.imageBufferFront.data = imagesData + 1;

		defaultFramebuffer.depthBuffer.data = myMallocT<DepthImage32>();
...
#endif // __GNUC__
	msdInit();
	cntxt->smallFramebuffer.imageBuffer.data = segImage;
	cntxt->smallFramebuffer.depthBuffer.data = segZBuff;
	cntxt->smallFramebuffer.setSize(WIDTH_SEG, HEIGHT_SEG);
	cntxt->segmentSize = { WIDTH_SEG, HEIGHT_SEG };
...
//TEXTURING_PART
	...
	halSyncAddr(&defaultFramebuffer, 0);
