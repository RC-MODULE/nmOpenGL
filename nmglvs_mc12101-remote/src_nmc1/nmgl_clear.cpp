...
SECTION(".text_demo3d") void NMGL_Clear(NMGL_Context_NM1 *context, NM_Command *command){
	NMGL_Framebuffer *frameBuffer = &defaultFramebuffer;
	int widthImage = frameBuffer->getWidth();
	int heightImage = frameBuffer->getHeight();
	//
	...
	case NMGL_COLOR_BUFFER_BIT: {
		if (x0 <= 0 && (width + x0) >= widthImage) {
			...
			for (int i = 0; i < size; i += STEP) {
				...
				int offset = i + origin;
				clearTasks[0].dst = (int*)frameBuffer->imageBuffer.data + offset;
			...
			for (int y = 0; y < height; y++) {
				int offset = (heightImage - (y + y0)) * widthImage + x0;
				clearTasks[0].dst = (int*)frameBuffer->imageBuffer.data + offset;
	...
	case NMGL_DEPTH_BUFFER_BIT: {
		if (x0 <= 0 && (width + x0) >= widthImage) {
			...
			for (int i = 0; i < size; i += STEP) {
				int localSize = MIN(STEP, size - i);
				int offset = i + origin;
				clearTasks[1].dst = (int*)frameBuffer->depthBuffer.data + offset;
