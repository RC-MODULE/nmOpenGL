...
SECTION(".text_nmglvs") NMGL_Framebuffer *nmglvsNm0Init()
{
	halSleep(500);
	...
	NMGL_Framebuffer *framebufferNM0 = (NMGL_Framebuffer*)halSyncAddr(0, 1);

	nmglClearColor(0, 0, 0, 1.0f);
	nmglClearDepthf(1);
	nmglViewport(0, 0, framebufferNM0->getWidth(), framebufferNM0->getHeight());
	cntxt->windowInfo.imageSize.width = framebufferNM0->getWidth();
	cntxt->windowInfo.imageSize.height = framebufferNM0->getHeight();
	nmglScissor(0, 0, framebufferNM0->getWidth(), framebufferNM0->getHeight());
